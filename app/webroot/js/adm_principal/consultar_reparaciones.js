...
function buscar_solicitudes(frase_busqueda, criterio_fecha, fecha_1, fecha_2, mostrar_solicitudes, criterio_campo, tipo_servicio, criterio_oficina, criterio_funcionario)
{
	$.ajax(
	{
		type: "POST",
		...
		success: function(datos_json)
		{
			if ( datos_json.resultado == true )
			{
				actualizar_link_xls(frase_busqueda, criterio_fecha, fecha_1, fecha_2, mostrar_solicitudes, criterio_campo, tipo_servicio, criterio_oficina);
			}
			else
			{
...
jQuery(document).ready(function()
{
	...
	jQuery('#boton_buscar_solicitudes').click(function()
	{
		...
		else
		{
			...
			var criterio_funcionario = jQuery('#select_funcionario').val();
			var mostrar_solicitudes = jQuery('#select_solicitudes').val();
			var tipo_servicio = jQuery('#select_servicio').val();
			buscar_solicitudes(frase_busqueda, criterio_fecha, fecha_1, fecha_2, mostrar_solicitudes, criterio_campo, tipo_servicio, criterio_oficina, criterio_funcionario);
		}
	});
	jQuery('#select_campo').change(function()
	{
		...
		if ( jQuery(this).val() == 'id' )
		{
			...
			jQuery('#select_funcionario').attr('disabled', 'true');
		...
		else
		{
			...
			jQuery('#select_funcionario').removeAttr('disabled');
